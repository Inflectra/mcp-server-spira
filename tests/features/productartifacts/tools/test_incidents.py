...
class TestGetIncidentsImpl:
    ...
    async def test_sort_parameter(self):
        ...
        await _get_incidents_impl(mock_client, 55, sort_by="PriorityId")
...
class TestGetIncidentsMCPWrapper:
    ...
    async def test_successful_call_through_wrapper(self, mock_get_client):
        ...
        result = await get_incidents_wrapper(product_id=55)
    ...
    async def test_validation_error_invalid_product_id(self, mock_get_client):
        ...
        get_incidents_wrapper = capture_registered_tool(register_tools)
        result = await get_incidents_wrapper(product_id=-1)
    ...
    @patch("mcp_server_spira.features.productartifacts.tools.incidents.get_spira_client")
    @pytest.mark.asyncio
    async def test_validation_error_invalid_start_row(self, mock_get_client):
        """Test validation error for invalid start_row."""
        get_incidents_wrapper = capture_registered_tool(register_tools)
        result = await get_incidents_wrapper(product_id=55, start_row=0)
    ...
    @patch("mcp_server_spira.features.productartifacts.tools.incidents.get_spira_client")
    @pytest.mark.asyncio
    async def test_validation_error_invalid_number_rows(self, mock_get_client):
        """Test validation error for invalid number_rows."""
        get_incidents_wrapper = capture_registered_tool(register_tools)
        result = await get_incidents_wrapper(product_id=55, number_rows=0)
    ...
    async def test_all_parameters_passed_through(self, mock_get_client):
        ...
        get_incidents_wrapper = capture_registered_tool(register_tools)
        await get_incidents_wrapper(
            product_id=55, start_row=10, number_rows=50, sort_by="PriorityId"
        )
