...
from mcp_server_spira.features.mywork.tools.mytasks import (
    _get_my_tasks_impl,
    get_spira_client,
)
...
class TestGetMyTasksImpl:
    ...
    async def test_successful_retrieval_with_default_pagination(self):
        """Test successful task retrieval with default pagination."""
        # Mock Spira client
        mock_client = AsyncMock()
        mock_tasks = [
            {"TaskId": i, "Name": f"Task {i}", "TaskStatusName": "In Progress"}
            for i in range(1, 51)
        ]
        mock_client.make_spira_api_get_request.return_value = mock_tasks

        # Call implementation with defaults
        result = await _get_my_tasks_impl(mock_client, limit=25, offset=0)

        # Verify API was called correctly
        mock_client.make_spira_api_get_request.assert_called_once_with("tasks")
        ...
        assert parsed["data"][0]["TaskId"] == 1
        assert parsed["data"][24]["TaskId"] == 25
    ...
    async def test_successful_retrieval_first_page(self):
        ...
        mock_client = AsyncMock()
        mock_tasks = [{"TaskId": i, "Name": f"Task {i}"} for i in range(1, 101)]
        mock_client.make_spira_api_get_request.return_value = mock_tasks
    ...
    async def test_successful_retrieval_middle_page(self):
        ...
        result = await _get_my_tasks_impl(mock_client, limit=25, offset=25)

        parsed = json.loads(result)
        assert len(parsed["data"]) == 25
        assert parsed["data"][0]["TaskId"] == 26
        assert parsed["data"][24]["TaskId"] == 50
    ...
    async def test_successful_retrieval_last_page_full(self):
        ...
        result = await _get_my_tasks_impl(mock_client, limit=25, offset=75)

        parsed = json.loads(result)
        assert len(parsed["data"]) == 25
        assert parsed["data"][0]["TaskId"] == 76
        assert parsed["data"][24]["TaskId"] == 100
    ...
    async def test_successful_retrieval_last_page_partial(self):
        ...
        mock_client = AsyncMock()
        mock_tasks = [{"TaskId": i, "Name": f"Task {i}"} for i in range(1, 48)]
        mock_client.make_spira_api_get_request.return_value = mock_tasks
        ...
        assert parsed["data"][0]["TaskId"] == 26
        assert parsed["data"][21]["TaskId"] == 47
    ...
    async def test_empty_results_with_offset(self):
        ...
        mock_client = AsyncMock()
        mock_tasks = [{"TaskId": i, "Name": f"Task {i}"} for i in range(1, 26)]
        mock_client.make_spira_api_get_request.return_value = mock_tasks

        result = await _get_my_tasks_impl(mock_client, limit=25, offset=50)
    ...
    async def test_custom_limit(self):
        ...
        result = await _get_my_tasks_impl(mock_client, limit=50, offset=0)
    ...
    async def test_limit_larger_than_total(self):
        ...
        mock_client = AsyncMock()
        mock_tasks = [{"TaskId": i, "Name": f"Task {i}"} for i in range(1, 11)]
        mock_client.make_spira_api_get_request.return_value = mock_tasks

        result = await _get_my_tasks_impl(mock_client, limit=100, offset=0)
    ...
    async def test_single_task(self):
        """Test with single task result."""
        mock_client = AsyncMock()
        mock_tasks = [{"TaskId": 1, "Name": "Single Task"}]
        mock_client.make_spira_api_get_request.return_value = mock_tasks
    ...
    @pytest.mark.asyncio
    async def test_preserves_task_data_structure(self):
        """Test that all task fields are preserved in response."""
        mock_client = AsyncMock()
        mock_tasks = [
            {
                "TaskId": 123,
        ...
        assert task["CustomProperties"] == [{"id": 1, "value": "test"}]
        assert task["Tags"] == "bug,security"
    ...
    async def test_json_structure_validity(self):
        ...
        mock_client = AsyncMock()
        mock_tasks = [{"TaskId": 1, "Name": "Task 1"}]
        mock_client.make_spira_api_get_request.return_value = mock_tasks
    ...
    async def test_pagination_metadata_accuracy(self):
        ...
        mock_client = AsyncMock()
        mock_tasks = [{"TaskId": i} for i in range(1, 76)]
        mock_client.make_spira_api_get_request.return_value = mock_tasks
        ...
        for limit, offset, expected_returned, expected_total, expected_has_more in test_cases:
            result = await _get_my_tasks_impl(mock_client, limit=limit, offset=offset)
            parsed = json.loads(result)
...
class TestGetMyTasksToolIntegration:
    ...
    @patch("mcp_server_spira.features.mywork.tools.mytasks.get_spira_client")
    @pytest.mark.asyncio
    async def test_validation_limit_too_high(self, mock_get_client):
        """Test that limit validation rejects values > 500."""

        from mcp_server_spira.features.mywork.tools.mytasks import register_tools
    ...
    async def test_validation_passes_with_valid_params(self, mock_get_client):
        ...
        mock_client = AsyncMock()
        mock_client.make_spira_api_get_request.return_value = [{"TaskId": 1, "Name": "Task 1"}]
...
class TestGetMyTasksRealAPIIntegration:
    ...
    async def test_returns_valid_json_structure(self, spira_client):
        ...
        print(f"  Total tasks: {pagination['total_count']}")
    ...
    async def test_pagination_works_with_real_data(self, spira_client):
        ...
        # Get first page
        result1 = await _get_my_tasks_impl(spira_client, limit=5, offset=0)
        parsed1 = json.loads(result1)

        # Get second page
        result2 = await _get_my_tasks_impl(spira_client, limit=5, offset=5)
        parsed2 = json.loads(result2)

        # If there are enough tasks, verify pages are different
        if (
            parsed1["pagination"]["total_count"] > 5
            and len(parsed1["data"]) > 0
            and len(parsed2["data"]) > 0
            and parsed1["data"][0]["TaskId"] != parsed2["data"][0]["TaskId"]
        ):
            print("\n✓ Pagination test passed:")
            print(f"  Page 1 first task: {parsed1['data'][0]['TaskId']}")
            print(f"  Page 2 first task: {parsed2['data'][0]['TaskId']}")
    ...
    async def test_handles_empty_results(self, spira_client):
        ...
        result = await _get_my_tasks_impl(spira_client, limit=25, offset=10000)
        parsed = json.loads(result)
    ...
    @pytest.mark.asyncio
    async def test_preserves_task_fields(self, spira_client):
        """Test that all task fields are preserved."""
        ...
        if len(parsed["data"]) > 0:
            ...
            expected_fields = ["TaskId", "Name"]
            for field in expected_fields:
                assert field in task, f"Missing field: {field}"

            print("\n✓ Field preservation test passed:")
            print(f"  Task has {len(task.keys())} fields")
            print(f"  Sample fields: {list(task.keys())[:10]}")
