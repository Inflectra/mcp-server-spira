...
| Tool                        | What it does                                                        |
| --------------------------- | ------------------------------------------------------------------- |
| `product_search_artifacts`  | Search requirements (and any other artifact type) in a product      |
| `product_get_artifact`      | Retrieve a single requirement by ID, optionally with inline steps   |
| `mywork_search_artifacts`   | List requirements assigned to the current user                      |
...
```json
{
  ...
  "fields_returned": ["RequirementId", "Name", "StatusName"],
  "fields_available": ["Description", "EstimatePoints", "..."],
  "pagination": {"starting_row": 1, "number_of_rows": 100, "total_returned": 42},
  "warnings": []
}
```

Use `fields` to request specific fields, or omit it to get the default summary fields. Call `get_artifact_schema(artifact_type="requirement")` to discover all available field names.

### Inline sub-artifact data

`product_get_artifact` supports an `include` parameter for requirements:

```
product_get_artifact(artifact_type="requirement", artifact_id=12, product_id=55, include=["steps"])
```

This returns the requirement with its use-case scenario steps embedded inline — no separate call needed.

## Common Workflows

### Check overall requirements status for a product
```
product_search_artifacts(artifact_type="requirement", product_ids=[55])
```
...
### Filter by status
```
product_search_artifacts(artifact_type="requirement", product_ids=[55], status="in progress")
```
`status` is a case-insensitive substring filter — `"in progress"` matches any status name containing that phrase.

### Get details of a specific requirement
```
product_get_artifact(artifact_type="requirement", artifact_id=12, product_id=55)
```
Returns the full requirement object including all fields, status, owner, release assignment, and estimate.

### Get a requirement with its use-case steps
```
product_get_artifact(artifact_type="requirement", artifact_id=12, product_id=55, include=["steps"])
```

### List requirements assigned to the current user
```
mywork_search_artifacts(artifact_type=["requirement"])
```
Supports the same `status`, `priority`, `fields`, `limit`, and `offset` parameters.

### Search across multiple products
```
product_search_artifacts(artifact_type="requirement", product_ids=[55, 56], status="accepted")
```
